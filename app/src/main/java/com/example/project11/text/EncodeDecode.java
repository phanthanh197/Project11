...
class EncodeDecode {
            ...
            for (int col = 0; col < image_columns; col++) {
                ...
                for (int channelIndex = 0; channelIndex < channels; channelIndex++) {
                    ...
                    if (!messageEncodingStatus.isMessageEncoded()) {
                        ...
                        // Thay đổi giá trị số nguyên bằng 2 ở bên trái và thay thế hai chữ số có nghĩa nhỏ nhất bằng các giá trị message_byte_array ..
                        ...
                        //sao chép số nguyên vào mảng kết quả
    ...
    public static List<Bitmap> encodeMessage(List<Bitmap> splitted_images,
        ...
        //tạo một List với số phần tử
        List<Bitmap> result = new ArrayList<>(splitted_images.size());

        //thêm kí tự vào đầu và cuối tin nhắn đc mã hóa
        encrypted_message = START_MESSAGE_COSTANT + encrypted_message + END_MESSAGE_COSTANT;

        //lấy mảng byte từ String
        byte[] byte_encrypted_message = encrypted_message.getBytes(Charset.forName("ISO-8859-1"));

        //trạng thái mã hoa tin nhắn
        ...
        for (Bitmap bitmap : splitted_images) {
            ...
            if (!message.isMessageEncoded()) {
                ...
                //lấy chiều cao và rông của bitmap
                ...
                //chuyển mang các byte_image thành mảng các số nguyên
                int[] oneDMod = Utility.byteArrayToIntArray(encodedImage);

                //tạo bit map từ encrypted_image_array
                Bitmap encoded_Bitmap = Bitmap.createBitmap(width, height,
                        Bitmap.Config.ARGB_8888);//ARGB_8888 mỗi pixel chia làm 4 byte
                ...
                for (int j = 0; j < height; j++)
                    for (int i = 0; i < width; i++) {
                        ...
                        //&0xFF trả về giá trị thập phân tương đương với 8 bit cuối cùng
                        // dịch bit với >>
                        //vd: 1000010001100011>>2&0xFF=100001.00011000&0xFF=24
